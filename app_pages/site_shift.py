...
if not artifacts_are_available():
    ...
    st.dataframe(
        ...
        hide_index=True,
        column_config={
            "mean_age": st.column_config.NumberColumn("Mean age", format="%.2f"),
            "asd_pct": st.column_config.NumberColumn("ASD proportion", format="%.1f%%"),
            "mean_full_scale_iq": st.column_config.NumberColumn("Mean FIQ", format="%.2f"),
        },
    )
